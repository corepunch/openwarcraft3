...
- The current packed WoW `equipment` bytes are local display-kit selectors, not raw item IDs. Resolve those kit selectors through DBC-backed `ItemDisplayInfo` display IDs in WoW-owned renderer/tool code, and keep the game state packed with `Wow_PackEquipment(...)` rather than widening entity/player state for preview gear.
