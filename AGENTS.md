...
- **Flat ability definitions.** Keep spell metadata and validate/execute callbacks directly in `ability_t`. `AB_SPELL_SIMPLE` selects the shared cast processor; combine independent policies such as `AB_CHANNEL`, `AB_TOGGLE`, and `AB_AUTOCAST`. Shared-cast abilities omit `.cmd`; use `S_AbilityHasCommand` and `S_AbilityCommand` for command capability and dispatch. Do not add runtime parents or inherited callback tables. Use the TFT class registry as an offline behavior reference and name concrete globals after TFT classes (`CAbilityHolyBolt`). The `SPELL`, `HUMAN_SPELL`, and `CAMPAIGN_SPELL` macros prepend `C` to the PascalCase name argument. Inert registry definitions preserve existing ability indices without implementing base classes. Run `python3 tools/wc3_ability_class_audit.py --format=coverage` to audit registration against the TFT registry; registration alone does not prove implemented behavior.
