...
- **Ability class hierarchy.** Every `ability_t` has a `.parent` pointer that mirrors the TFT `CAbility*` class tree extracted from `tft-ability-classes.txt`. Abstract base classes are defined in `skills/s_ability_classes.c`; concrete abilities set their parent in `S_WireAbilityParents()` called from `InitAbilities`. Name every ability global after its TFT class: `CAbilityHolyBolt`, `CAbilityMorph`, `CAbilitySimpleSpell`. The `SPELL`, `HUMAN_SPELL`, and `CAMPAIGN_SPELL` macros prepend `C` to the PascalCase name argument (e.g. `SPELL(AbilityDoom, ...)` produces `ability_t CAbilityDoom`). When adding a new ability, look up its parent in `tft-ability-classes.txt` and wire `.parent` to the correct base class or concrete parent ability — for example `CAbilityFireBolt.parent = &CAbilityThunderBolt` because `ANfb` inherits from `AHtb`. Run `python3 tools/wc3_ability_class_audit.py --format=coverage` to track implementation progress against the TFT class registry.
