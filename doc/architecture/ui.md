...
As of Phase 8 (May 2026), all UI logic in OpenWarcraft3 runs **client-side** in the UI library (`ui/`). The server is now game-agnostic and provides unit data through a query protocol when clients request it. This follows the Quake 3 Arena pattern where UI is a separate client-side library.
...
`UI_Init` (`ui/ui_main.c`) is called once from `CL_Init` when the client starts:
...
FDF files declare frame templates hierarchically. `UI_ParseFDF_Buffer` (`ui/ui_fdf.c`) accepts a writable C string, tokenises it, and registers `frameDef_t` entries in a global lookup table. Frames can inherit from a base template with `InheritsParts`.
...
1. In the appropriate screen controller (e.g., `ui/screens/console_ui.c`), define the frame using the FDF API or programmatically:
...
| `ui/ui_main.c` | `UI_GetAPI`, library entry point, screen routing |
| `ui/ui_fdf.c` | FDF parser and programmatic frame API |
| `ui/ui_render.c` | Frame rendering dispatch |
| `ui/screens/console_ui.c` | In-game HUD (resource bar, command card, inventory) |
| `ui/screens/main_menu.c` | Main menu, single player menu, etc. |
