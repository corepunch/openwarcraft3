...
/* -------------------------------------------------------------------------
 * Character-creation DBC helpers
 * ---------------------------------------------------------------------- */

/* Shared DBC state loaded once on first use. */
typedef struct { DWORD id; DWORD faction_id; DWORD flags; DWORD male_id; DWORD female_id;
                 LPCSTR client_prefix; LPCSTR client_file; LPCSTR name; LPCSTR name_female;
                 LPCSTR hair_custom; LPCSTR facial_custom[2]; DWORD required_exp; } wowRaceRec_t;
...
static struct {
    ...
    int  sel_race;   /* 0-based index into races[] (playable races in faction order) */
    int  sel_sex;    /* 1 = male, 2 = female (Lua convention) */
    int  sel_class;  /* 1-based index into classes[] */
    float facing;
    /* playable race list (indices into races[]) */
...
static void UIWow_LoadCharCreateDbc(void) {
    ...
    /* ChrRaces — fields: 0=id,1=flags,2=factionID,3=explorationSnd,4=maleDID,5=femaleDID,
       6=clientPrefix(str),7=baseLang,8=creatureType,9=resSickSpell,10=splashSnd,
       11=clientFileString(str),12=cinematicSeq,13=alliance,14=name(str),15=nameFemale(str),
       16=nameMale(str),17=facialHairCustom0(str),18=facialHairCustom1(str),19=hairCustom(str),
    ...
    if (data) {
        ...
        FOR_LOOP(i, (int)records) {
            ...
            if (flags & 0x1) continue; /* NPC-only race */
    ...
    /* ChrClasses — fields: 0=id,1=damageBonusStat,2=displayPower,3=petNameToken(str),
    ...
    /* FactionTemplate — fields: 0=id,1=faction,2=flags,3=factionGroup,...  */
    ...
    /* FactionGroup — fields: 0=id,1=maskID,2=internalName(str),3=name(str) */
    ...
    /* Build playable race list: Alliance first, then Horde (matches whoa) */
    ...
    FOR_LOOP(side, 2) {
        FOR_LOOP(ri, wow_charcreate.num_races) {
            ...
            /* find faction template for this race */
    ...
    wow_charcreate.sel_sex   = 1; /* male */
    wow_charcreate.sel_class = 1;
    wow_charcreate.facing    = -15.0f;
}

/* look up faction group name for a race index (1-based) */
...
/* ---- Lua-exposed character-create functions ---- */

static int UIWow_LuaGetAvailableRaces(lua_State *L) {
    UIWow_LoadCharCreateDbc();
...
static int UIWow_LuaGetAvailableClasses(lua_State *L) {
    UIWow_LoadCharCreateDbc();
...
static int UIWow_LuaGetClassesForRace(lua_State *L) {
    UIWow_LoadCharCreateDbc();
    int pi = wow_charcreate.sel_race; /* 0-based playable index */
...
static int UIWow_LuaGetFactionForRace(lua_State *L) {
    UIWow_LoadCharCreateDbc();
...
static int UIWow_LuaGetNameForRace(lua_State *L) {
    UIWow_LoadCharCreateDbc();
...
static int UIWow_LuaGetSelectedRace(lua_State *L) {
    UIWow_LoadCharCreateDbc();
    lua_pushnumber(L, wow_charcreate.sel_race + 1);
    return 1;
}
static int UIWow_LuaGetSelectedSex(lua_State *L) {
    UIWow_LoadCharCreateDbc();
    lua_pushnumber(L, wow_charcreate.sel_sex);
    return 1;
}
static int UIWow_LuaGetSelectedClass(lua_State *L) {
    UIWow_LoadCharCreateDbc();
...
static int UIWow_LuaGetHairCustomization(lua_State *L) {
    UIWow_LoadCharCreateDbc();
    int pi = wow_charcreate.sel_race;
...
static int UIWow_LuaGetFacialHairCustomization(lua_State *L) {
    UIWow_LoadCharCreateDbc();
    int pi = wow_charcreate.sel_race;
...
static int UIWow_LuaGetCharacterCreateFacing(lua_State *L) {
    UIWow_LoadCharCreateDbc();
    lua_pushnumber(L, wow_charcreate.facing);
    return 1;
}
static int UIWow_LuaSetCharacterCreateFacing(lua_State *L) {
    UIWow_LoadCharCreateDbc();
    wow_charcreate.facing = (float)luaL_checknumber(L, 1);
    return 0;
}
static int UIWow_LuaResetCharCustomize(lua_State *L) {
    (void)L;
    UIWow_LoadCharCreateDbc();
    wow_charcreate.sel_race  = 0;
...
static int UIWow_LuaGetRandomName(lua_State *L) {
    (void)L; lua_pushstring(L, ""); return 1;
}
static int UIWow_LuaCreateCharacter(lua_State *L) {
    (void)L; return 0;
}
