...
typedef struct mdxMaterialLayer_s {
    ...
    mdxGeoFlags_t flags;
    dword_t textureId;        // TEXS index or 0xFFFFFFFF for none
    dword_t transformId;      // TXAN index or 0xFFFFFFFF for none
...
typedef struct mdxNode_s {
    mdxObjectName_t name;
    dword_t object_id; // globally unique id, used as the index in the hierarchy. index into PIVT
    dword_t parent_id; // parent MDLGENOBJECT's objectId or 0xFFFFFFFF if none
    dword_t flags;
