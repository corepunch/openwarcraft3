...
ENTRY = re.compile(r'^\s*\{\s*"([A-Za-z0-9]{4})"\s*,\s*([^}]+?)\s*\},(.*)$')
TODO_ENTRY = re.compile(r'^\s*// TODO:\s*([A-Za-z0-9]{4})\s+(\w+)(.*)$')
OLD_TODO_ENTRY = re.compile(r'^\s*// TODO:\s*\{\s*"([A-Za-z0-9]{4})"\s*,\s*([^}]+?)\s*\},(.*)$')
HEADER = re.compile(r"^\[([^]\r\n]{4})\]\s*$")
...
def load_sources(strings_dir: Path):
    ...
    if not files:
        raise ValueError(f"no *AbilityStrings.txt files in {strings_dir}")
    sources = {}
...
def registry_region(text: str):
    ...
    if not todo_entries:
        todo_entries = parse_entries(text[todo_start:todo_finish], OLD_TODO_ENTRY)
    if not entries:
...
def render_group(lines, entries, file_names, sources, todo=False):
    ...
    other = []
    for rawcode, definition, old_comment in entries:
        source = sources.get(rawcode)
        if source:
            file_index, order, display_name, _ = source
            groups[file_index].append((order, rawcode, definition, display_name))
        else:
                other.append((rawcode, definition, FALLBACK_NAMES.get(rawcode, old_comment or rawcode)))
    for file_index, file_name in enumerate(file_names):
        group = sorted(groups[file_index])
        if not group:
            continue
        lines.extend(["", f"    /* {file_name} */"])
        for _, rawcode, definition, display_name in group:
            if todo:
                lines.append(f'    // TODO: {rawcode} {definition.removeprefix("&")}  /* {display_name} */')
            else:
                lines.append(f'    {{ "{rawcode}", {definition} }},  /* {display_name} */')
    if other:
        lines.extend(["", "    /* No AbilityStrings source file */"])
        for rawcode, definition, comment in sorted(other):
            if todo:
                lines.append(f'    // TODO: {rawcode} {definition.removeprefix("&")}  /* {comment} */')
            else:
                lines.append(f'    {{ "{rawcode}", {definition} }},  /* {comment} */')
...
def render(text: str, strings_dir: Path) -> str:
    ...
    active_codes = {rawcode for rawcode, _, _ in entries}
    if len(active_codes) != len(entries):
        raise ValueError("generated active registry contains duplicate rawcodes")
    seen = set(active_codes)
    todo_entries = [entry for entry in todo_entries if entry[0] not in seen and not seen.add(entry[0])]
