...
Examples:
  ...
  python3 tools/wc3_ability_class_audit.py --format=parents  # show offline retail parent relationships
...
def format_parents(classes, active, todo):
    """Print retail parent relationships as offline reference, not runtime wiring."""
    ...
    for varname in sorted(var_fourcc):
        ...
        if parent_fc and parent_fc in classes:
            ...
            print(f"{fourcc}: {cn} -> {parent_cn} ({parent_fc})")
