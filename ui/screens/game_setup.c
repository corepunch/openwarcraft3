...
static void GameSetup_EnableSlotControl(LPFRAMEDEF row, LPCSTR name) {
...
static void GameSetup_BindSlotRow(gameSetupSlotRow_t *slot) {
    ...
    name_menu = UI_FindChildFrame(slot->row, "NameMenu");
    race_menu = UI_FindChildFrame(slot->row, "RaceMenu");
    handicap_menu = UI_FindChildFrame(slot->row, "HandicapMenu");

    GameSetup_FitSlotRow(slot->row);
    slot->name_text = GameSetup_FindPopupTitleText(name_menu);
    slot->race_text = GameSetup_FindPopupTitleText(race_menu);
    slot->team_text = UI_FindChildFrame(slot->row, "TeamButtonTitle");
    slot->color_value = GameSetup_EnsureColorValue(slot->row);

    GameSetup_PositionPopupMenuParts(name_menu);
    GameSetup_PositionPopupMenuParts(race_menu);
    GameSetup_EnableSlotControl(slot->row, "NameMenu");
    GameSetup_EnableSlotControl(slot->row, "RaceMenu");
    GameSetup_EnableSlotControl(slot->row, "TeamButton");
    GameSetup_EnableSlotControl(slot->row, "ColorButton");
    GameSetup_HideSlotChild(slot->row, "NamePopupMenuMenu");
    GameSetup_HideSlotChild(slot->row, "RacePopupMenuMenu");
...
static void GameSetup_AddSlot(DWORD row_index,
                              LPCSTR name,
                              LPCSTR race,
                              LPCSTR team,
                              DWORD color_index)
{
    ...
    GameSetup_SetTextIfPresent(row->name_text, "%s", name ? name : "");
    GameSetup_SetTextIfPresent(row->race_text, "%s", race ? race : "");
    GameSetup_SetTextIfPresent(row->team_text, "%s", team ? team : "");
    if (row->color_value) {
...
static void GameSetup_PopulateSlots(void) {
    ...
    FOR_LOOP(i, MAX_PLAYERS) {
        ...
        if (setup.map_info.teams &&
            force_index < setup.map_info.num_teams &&
            setup.map_info.teams[force_index].name &&
            setup.map_info.teams[force_index].name[0]) {
            GameSetup_ResolveMapString(setup.map_info.teams[force_index].name, team, sizeof(team));
            if (!team[0]) {
                snprintf(team, sizeof(team), "Team %u", (unsigned)force_index + 1);
            }
        } else {
            snprintf(team, sizeof(team), "Team %u", (unsigned)force_index + 1);
        }
        ...
        GameSetup_AddSlot(visible,
                          name,
                          GameSetup_RaceName(player->playerRace),
                          team,
                          i);
        visible++;
    }
